...
def registrar_movimiento():
    ...
    if request.method == 'POST':
        ...
        conn, cursor = conectar_bd()
        cursor.execute(
            "SELECT id, saldo FROM cuentas WHERE nombre = ?",
            (cuenta,)
        )
        cuenta_row = cursor.fetchone()
        if cuenta_row:
            cuenta_id, saldo_actual = cuenta_row
            if tipo == 'Ingreso':
            ...
            cursor.execute(
                "INSERT INTO movimientos (fecha, cuenta_id, tipo, monto) "
                "VALUES (?, ?, ?, ?)",
                (fecha, cuenta_id, tipo, monto)
            )
            cursor.execute(
...
def dashboard():
    conn, cursor = conectar_bd()
    cursor.execute(
        "SELECT tipo, nombre, saldo FROM cuentas ORDER BY tipo, nombre"
    )
    cuentas = cursor.fetchall()
    ...
    cursor.execute(
        ...
        cuentas.nombre, movimientos.tipo,
        movimientos.monto
        FROM movimientos
    ...
    saldos_por_mes = {
        mes: {cuenta: 0 for cuenta in cuentas_nombres} for mes in meses
    }
    ...
    datasets = [
        {
            ...
            'data': [saldos_por_mes[mes][cuenta] for mes in meses]
        }
        for i, cuenta in enumerate(cuentas_nombres)
    ]
...
def historial():
    conn, cursor = conectar_bd()
    cursor.execute('''
        SELECT movimientos.fecha, cuentas.nombre, movimientos.tipo,
        movimientos.monto
        FROM movimientos
        JOIN cuentas ON movimientos.cuenta_id = cuentas.id
...
def leer_csvs():
    ...
    fechas = sorted(datos_csv.keys())
    cuentas = sorted(
        set(c for f in datos_csv.values() for c in f)
    )
    datos_procesados = {
        "fechas": fechas,
        "cuentas": cuentas,
        "saldos": [
            [datos_csv[fecha].get(cuenta, 0) for cuenta in cuentas]
            for fecha in fechas
        ],
    }
